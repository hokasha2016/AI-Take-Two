...
Code retrieved from Github, by a user named Sonph (Son A. Pham)
...
class TestBayesNet(unittest.TestCase):
    ...
    def test_normalize0(self):
        #tests the nomralization method
    ...
    #tests the alarm.bn query with different cases
        cases = [
            (('B', {'B': False}), .999),
            (('B', {'B': True}), .001),
    ...
    def test_querygiven_ex2(self):
        # tests the ex2.bn query with different cases
        cases = [
            (('A', {'A': False}), 0.7),
            (('B', {'B': True}), 0.6),
